...
png_dir=sprintf('%s/Histology/%s/%dum_png',hist_dir,subj,png_res);

coreg_dir=sprintf('%s/Histology_Hp/%s/%dum_png.coregHE',hist_dir,subj,png_res);
stains={'HE','NEUN','GFAP','LFB','LUXFB'};

out_dir=sprintf('%s/aligned_rgb_%dum_Hp',reg_dir,png_res);
...
for slice=1:nslices
    ...
    if (~exist(in_png) && exist(next_png) )
