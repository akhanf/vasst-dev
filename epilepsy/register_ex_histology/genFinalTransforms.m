...
for slice=1:nslices
    ...
    hist_slice_alt=sprintf('%s/images/hist_slice_%02d.nii.gz',reg_dir,slice);
...
for slice=1+totaloffset:nslices+totaloffset
    ...
    mri_vol=sprintf('%s/images/ex_mri_th50_iso_crop.nii.gz',reg_dir);
    if ~exist(mri_vol)
    ...
    final_phys=sprintf('%s/ex-hist_slice_%02d.xfm',final_dir,slice);
    final_phys_slicer=sprintf('%s/ex-hist_slice_%02d.tfm',final_dir,slice);

    dlmwrite(final_phys,final_xfm,' ');
    convertRAStoSlicerTfm(final_xfm,final_phys_slicer);
    
    %write inverses too
    final_phys_inv=sprintf('%s/hist-ex_slice_%02d.xfm',final_dir,slice);
    final_phys_slicer_inv=sprintf('%s/hist-ex_slice_%02d.tfm',final_dir,slice);
    ...
    aligned_phys=sprintf('%s/aligned-hist_slice_%02d.xfm',final_dir,slice);
    aligned_phys_slicer=sprintf('%s/aligned-hist_slice_%02d.tfm',final_dir,slice);

    aligned_phys_inv=sprintf('%s/hist-aligned_slice_%02d.xfm',final_dir,slice);
    aligned_phys_slicer_inv=sprintf('%s/hist-aligned_slice_%02d.tfm',final_dir,slice);
