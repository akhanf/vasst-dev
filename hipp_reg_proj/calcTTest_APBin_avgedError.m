...
%% if comparing t1/t2 to t1/t2, run comparison for both sides
    ...
    avg_dist_L_comb1=mean(dist_mat_L_comb1,1);
    avg_dist_L_comb2=mean(dist_mat_L_comb2,1);
    avg_dist_R_comb1=mean(dist_mat_R_comb1,1);
    avg_dist_R_comb2=mean(dist_mat_R_comb2,1);
 
    for i=1:12
        val=((i - 1) * 11 + 1);
        vec_L_comb1(i,1)=mean(avg_dist_L_comb1(val:val+10));
        vec_L_comb2(i,1)=mean(avg_dist_L_comb2(val:val+10));
        vec_R_comb1(i,1)=mean(avg_dist_R_comb1(val:val+10));
        vec_R_comb2(i,1)=mean(avg_dist_R_comb2(val:val+10));
    end
