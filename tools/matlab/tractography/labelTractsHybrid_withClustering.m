...
%data_dir=sprintf('%s/dti/distortCorrect/caminoTractographyDTI',subj);
...
rep_labels=[];
    ...
    for k=1:length(all_mean_bundles);
        if(isempty(all_mean_bundles{k}))
            emptyinds=[emptyinds; k];
        else
            nemptyinds=[nemptyinds; k];
            rep_labels=[rep_labels; repmat(all_mean_labels(k),size(all_mean_bundles{k},1),1)];
        end
    end
    
writeTractsToVTK(all_mean_bundles(nemptyinds),rep_labels,sprintf('%s/representative_tracts.vtk',bundle_dir));
...
for rb=1:length(rep_labels)
  %  optimal_affinity.data(find(strcmp(rep_names{rb},optimal_affinity.textdata)))
...
%first attempt was to weight affinity -- but then we lose some tracts
%completey -- better to weight the sigma
%A_scaled=A.* repmat(affinity_scaling',N,1);
...
%% write all tracts, painted by affinity to a given representative tract
...
[maxval,maxind]=max(combined_cost,[],2);
   ...
   dist_threshold=0.5;
    ...
    rep_labels=cat(1,mean_bundles_label{:});
    
    %index to rep tract
    for rep_i=1:size(cost_dist,2)
        rep_name=bundle_name{rep_labels(rep_i)};
    
    cost_rep=cost_dist(maxval>dist_threshold,rep_i);
    ...
    for i=1:N
        ...
        endind=startind+length(selected_tracts{i})-1;
        scalars(startind:endind)= ones(length(selected_tracts{i}),1).*cost_rep(i);
        startind=endind+1;
    end
    labeled_out_vtk=sprintf('%s/00_tracts_repBundle_%02d_%s_th%02d.vtk',bundle_dir,rep_i,rep_name,dist_threshold);
...
%% write tracts that are far from any cluster mean
% 
% outlier_tract_threshold=30;
% 
% 
% [minval,minind]=min(dist_matrix,[],2);
% selected_tracts=all_tracts(minval>outlier_tract_threshold);
% scalars=minval(minval>outlier_tract_threshold);
% labeled_out_vtk=sprintf('%s/00_tracts_distFiltToMean_outlierTracts%d.vtk',bundle_dir,outlier_tract_threshold);
% writeTractsToVTK(selected_tracts,scalars,labeled_out_vtk);
% 

%% save label.csv file for BrightMatter, assign labels based on specified threshold for now,
% using label 100 as black with alpha 255


%using dist threshold alone discards the AND/NOT bundles completely.. 
    %should have a weighted combination
            %1 )  initial bundling (based on AND/NOT)
            %2 )  distance to mean bundles  <-- right now only this
            %3 )  spatial prior
...
for  alpha_img_cost=0%:0.1:0.3%  0:0.025:0.05%0:0.02:0.1  %0.2:1%[0,5]%[0,3,5] %3;  %weight for image cost
...
for dist_threshold=0.6%0.5:0.1:0.9%[5,10]
    
tract_selection=maxval>dist_threshold;

selected_tracts=all_tracts(tract_selection);
selected_tracts_uid=all_tracts_uid(tract_selection);
maxind_selected_tracts=maxind(tract_selection);

% set to label 100 for background tracts
all_tracts_labels=ones(length(all_tracts),1).*100;
%all_tracts_labels=zeros(length(all_tracts),1);
all_tracts_labels(tract_selection)=bundle_label(all_mean_labels(maxind_selected_tracts));

%just include labelled tracts
%all_tracts_labels=bundle_label(all_mean_labels(minind_selected_tracts));

%write out CSV
%out_label_csv='test_labels.csv';
labels_dir='synaptive_csv'
mkdir(labels_dir);

out_label_csv=sprintf('%s/%s.WithPriorsAlpha%d.withProbPriorsBeta%d.affinityTH%d.maxClusDist%d.csv',labels_dir,subj,alpha_img_cost,beta_img_cost,dist_threshold,max_distance);


fid=fopen(out_label_csv,'w');
fprintf(fid,'UID, Label\n');
for i=1:length(all_tracts_labels)
    fprintf(fid,'%.0f, %d\n',all_tracts_uid(i),all_tracts_labels(i));
end
fclose(fid);
...
%label, no threshold
all_tracts_nothreshold_label=bundle_label(all_mean_labels(maxind));

out_nothresh_label_csv=sprintf('%s/%s.LabelNoThreshold.WithPriorsAlpha%d.withProbPriorsBeta%d.maxClusDist%d.csv',labels_dir,subj,alpha_img_cost,beta_img_cost,max_distance);


fid=fopen(out_nothresh_label_csv,'w');
fprintf(fid,'UID, Label\n');
for i=1:length(all_tracts_labels)
    fprintf(fid,'%.0f, %d\n',all_tracts_uid(i),all_tracts_nothreshold_label(i));
end
fclose(fid);
...
all_tracts_bundledist=maxval;

out_dist_csv=sprintf('%s/%s.BundleAffinity.WithPriorsAlpha%d.withProbPriorsBeta%d.maxClusDist%d.csv',labels_dir,subj,alpha_img_cost,beta_img_cost,max_distance);

fid=fopen(out_dist_csv,'w');
fprintf(fid,'UID, BundleAffinity\n');
for i=1:length(all_tracts_bundledist)
    fprintf(fid,'%.0f, %f\n',all_tracts_uid(i),all_tracts_bundledist(i));
end
fclose(fid);
...
out_label_csv=sprintf('%s/%s.WithPriorsAlpha%d.withProbPriorsBeta%d.weighted.affinityTH%d.maxClusDist%d.csv',labels_dir,subj,alpha_img_cost,beta_img_cost,dist_threshold,max_distance);
...
out_nothresh_label_csv=sprintf('%s/%s.LabelNoThreshold.WithPriorsAlpha%d.withProbPriorsBeta%d.weighted.maxClusDist%d.csv',labels_dir,subj,alpha_img_cost,beta_img_cost,max_distance);
...
out_dist_csv=sprintf('%s/%s.BundleAffinity.WithPriorsAlpha%d.withProbPriorsBeta%d.weighted.maxClusDist%d.csv',labels_dir,subj,alpha_img_cost,beta_img_cost,max_distance);
